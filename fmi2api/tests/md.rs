...
mod tests {
    ...
    use fmi2api::md::MdModelDescription;
    use quick_xml::de::from_str;

    use super::*;

    #[test]
    fn test_modelDescriptionParsing() {
        let parsed: MdModelDescription = from_str(MD).unwrap();

        let values: HashMap<String, u32> = parsed
            .model_variables
            .variables
            .iter()
            .map(|v| (v.name.to_owned(), v.value_reference))
            .collect();
        println!("pairs: {:?}", &values);
        assert!(parsed.guid == "77236337-210e-4e9c-8f2c-c1a0677db21b")
    }
}
